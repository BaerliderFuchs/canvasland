...
function find_entropy_cell(cells, tiles) {
  ...
  for (y = 0; y < cells.length; y++) {
    for (x = 0; x < cells[y].length; x++) {
      ...
      if (entropy_rnd < min) {
        // Found a possible cell that has the least probability.
        min = entropy_rnd;
        min_x = x;
        min_y = y;
      }
...
function propagate(start_pos, cells, tiles, rules) {
  ...
  while ((pos = stack.pop()) && passes < PROP_LMT) {
    ...
    for (idx = 0; idx < 4; idx++) {
      ...
      if (!get_dir_pos(pos, DIRS[idx], chk_pos)) {
        console.log("Direction out of bounds", chk_pos);
        continue;
        break;
      }
...
const PASS_LMT = 40; // How many passes to go through the matrix.
