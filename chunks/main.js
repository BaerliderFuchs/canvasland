...
/*   for (let y = 0; y < windowHeight / CHUNK_SIZE; y++) {
    for (let x = 0; x < windowWidth / CHUNK_SIZE; x++) {
      generateChunk(y,x);
    } 
  } */
  updatePixels();
...
function generateChunk(chunkY, chunkX) {
  for (let y = chunkY * CHUNK_SIZE; y < chunkY * CHUNK_SIZE + CHUNK_SIZE; y++) {
    for (let x = chunkX * CHUNK_SIZE; x < chunkX * CHUNK_SIZE + CHUNK_SIZE; x++) {
      let index = (x + y * windowWidth) * 4;
      let r = noise(x * NOISE_SCALE, y * NOISE_SCALE, time) * 255;
      ...
      if (r > SEA_LEVEL + 1 && DRAW_BORDERS) {
        ...
        if (JSON.stringify(leftNeighbour) !== "[0,0,0]" && JSON.stringify(topNeighbour) !== "[0,0,0]" && JSON.stringify(rightNeighbour) !== "[0,0,0]" && JSON.stringify(bottomNeighbour) !== "[0,0,0]" && JSON.stringify(pixel) !== JSON.stringify(leftNeighbour) && JSON.stringify(leftNeighbour) !== JSON.stringify(COLORS.BORDER) || JSON.stringify(pixel) !== JSON.stringify(topNeighbour) && JSON.stringify(topNeighbour) !== JSON.stringify(COLORS.BORDER)) color = COLORS.BORDER;
